...
def verify_authorization_header(auth_header: str) -> Tuple[List[str], FastAPIUser]:
    ...
    scopes = ["admin", "user"]  # You could for instance use the scopes provided in the JWT or request them by looking up the scopes with the 'sub' somewhere
    return scopes, user
...
if __name__ == '__main__':
    uvicorn.run('server:app', host="0.0.0.0", port=8080, reload=True, log_level="info")  # Starts the uvicorn ASGI