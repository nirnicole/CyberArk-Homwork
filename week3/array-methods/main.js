...
let filtered = vegetables.filter((v) => v.color === "orange")
...
const increaseIfGood = (p) =>
  p.goodPerformance ? (p.salary += 300) : p.goodPerformance

people.forEach(increaseIfGood)
...
const findCommentByID = (pid, cid) =>
  findById(pid).comments.find((c) => c.id === cid)
...
console.log(
  movies.some((m) => m.studio === "Marvel")
    ? "lets go watch blabla"
    : "lets stay home otherwise"
)

console.log(movies.every((m) => m.year > 2020) ? "fantastic stuff" : "Yawn")
...
//ex2:
let filteredArr = users.filter((u) => u.address.zipcode.charAt(0) == 5)
console.log(filteredArr)

//ex3:
let filteredMapedArr = users
  .filter((u) => u.address.zipcode.charAt(0) == 5)
  .map((u) => u.id)
console.log(filteredMapedArr)

//ex4:
let ex4 = users.map((u) => u.name).filter((n) => n.charAt(0) === "C")

console.log(ex4)

//ex5:
let ex5 = users.every((u) => u.address.city === "South Christy")
console.log(ex5)

//ex6:
let ex6 = users.find((u) => u.address.suite === "Apt. 950").company.name
console.log(ex6)
